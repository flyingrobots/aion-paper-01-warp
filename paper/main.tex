...
This paper introduces \emph{Worldline Algebra for Recursive Provenance Graphs} (WARPs), a minimal categorical
structure that captures nested graph-shaped state. WARPs unify syntax, control
flow, provenance, traces, and hierarchical system structure into one algebraic
...
\emph{Worldline Algebra for Recursive Provenance graph} (WARP).  Informally, you take an ordinary finite
...
The graph-theoretic and categorical foundations underlying WARP graphs draw on a long line of work in algebraic graph transformation, including the double-pushout approach and adhesive categories developed by Ehrig, Prange, Taentzer, Lack, Sobociński, and others. Related formalisms for nested or hierarchical graph structure include bigraphs, hierarchical state machines, and various “graph-of-graphs” encodings used in program analysis and heterogeneous network theory.

The contribution of this work is not the invention of graph rewriting or nesting per se, but the synthesis of these ideas into a single initial-algebraic object with deterministic multiway semantics, explicit provenance payloads, and a computable observer geometry. To our knowledge, these elements have not previously been unified within one framework.

\paragraph{Contributions.}
The specific contributions are:

\begin{enumerate}
  \item A precise inductive definition of Worldline Algebra for Recursive Provenance (WARP) graphs as the least
  ...
  \item A notion of morphism making WARPs into a category \textbf{WARP}, together
...
Section~\ref{sec:WARP-unfold} introduces finite unfoldings and recursion schemes.
Section~\ref{sec:WARP-morphisms} defines morphisms and the category
\textbf{WARP}.  Section~\ref{sec:WARP-relation} relates WARP graphs to ordinary graphs
and hypergraphs and sketches how this sets the stage for DPO rewriting in later
work.  Section~\ref{sec:WARP-notation} summarises the main notation.
...
The class $\WARP$ of Worldline Algebra for Recursive Provenance graphs is the least set closed under the
...
\]
in $\mathbf{Set}$ (or in $\mathbf{WARP}$ if you prefer).  Intuitively it records,
...
\subsection{Morphisms and the category of WARP graphs}
...
formalisms such as bigraphs~\cite{Milner2001} offer alternative semantics; WARPs
