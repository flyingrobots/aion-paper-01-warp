...
structure that captures nested graph-shaped state. Formally, a \WARP\ graph is a finite directed multigraph whose vertices and edges carry attached \WARP s, yielding a finite well-founded hierarchy. The ``worldline'' terminology anticipates the deterministic evolution of such states developed in subsequent papers; there, rewrite sequences trace causal histories that resemble worldlines in spacetime diagrams. This paper focuses on the static state object. \WARP s unify hierarchical system structure, syntax, control
...
This paper extracts a minimal canonical object for this job: \WARP\ graphs (Worldline Algebra for Recursive Provenance).  Earlier drafts and companion notes used the name \emph{recursive metagraphs} (RMGs); we adopt \WARP\ to avoid collisions with existing uses of ``RMG'' and to emphasise the provenance/worldline interpretation.  Informally, you take an ordinary finite
...
We call an individual object a \WARP\ graph; when unambiguous, simply a \WARP\ (plural \WARP s).
...
object.  Later papers will use \WARP s as the state space for deterministic
...
\begin{enumerate}
  ...
  \item A notion of morphism making \WARP s into a category $\mathbf{WARP}$, together
...
example.  Section~\ref{sec:WARP-initial} recasts \WARP s as an initial algebra.
...
graphs and hypergraphs.  Section~\ref{sec:WARP-outlook} briefly situates \WARP s in
the broader \AION{} programme.  Section~\ref{sec:WARP-notation} summarises the
main notation.

\paragraph{Proof Sketches and References.}
All categorical claims made here are standard consequences of the inductive definition and well-founded recursion. Detailed proofs follow directly from the theory of polynomial functors and algebraic graph transformation; see, e.g., \cite{MacLane98,AdamekRosicky94,EEPT06,EL97}.
...
\paragraph{Remark.}
Although we state skeletons as plain finite multigraphs for concreteness, the recursive construction lifts verbatim to richer skeleton categories (for example typed open graphs $\OGraphT$).  We will exploit this generality when developing rewriting and provenance semantics on \WARP s in later papers.
...
bare atom or a finite skeleton whose vertices and edges carry smaller \WARP s.
...
if, say, a syntax tree node needs to store profiling \WARP s.  In one object the
high-level call graph and nested payloads stay coherent.

The figure shows this recursion: tiny mini-graphs hang off each attachment to
remind the reader that attachments are full \WARP s, not mere labels.
\end{example}

\begin{figure}[H]
  \centering
  \DiagramCallGraphExample
  \caption{Worked example: call-graph skeleton with attached ASTs for $f$ and $g$, and provenance payload on the call edge. Tiny mini-graphs inside attachments make the recursive nesting explicit.}
...
useful to rephrase \WARP s as an initial algebra for a polynomial functor.  This
makes structural recursion and induction theorems essentially automatic; see \cite{MacLane98,AdamekRosicky94}.
...
Practically, this tells you that to define a function out of \WARP s, it suffices
to say:
...
operations on \WARP s later.

\section{Depth and Unfoldings}
\label{sec:WARP-unfold}

Although \WARP s are finite by definition, the attachment tree can be deep.  It is
...
\]
If the skeleton $S$ has no vertices or edges, we define $\depthOf(X)=1$.
\end{definition}
...
To use \WARP s as honest mathematical objects, we need a notion of morphism.
...
A \emph{morphism} $f : X \to Y$ of \WARP s is defined by structural
recursion on depth.
...
Given $f : X \to Y$ and $g : Y \to Z$, their composite $g\circ f : X \to Z$ has
components
\[
  (g\circ f)_V = g_V \circ f_V,\qquad (g\circ f)_E = g_E \circ f_E,
\]
and for each $v \in V_S$ and $e \in E_S$,
\[
  (g\circ f)_v = g_{f_V(v)} \circ f_v,\qquad (g\circ f)_e = g_{f_E(e)} \circ f_e.
\]
Identities are defined analogously.
\end{definition}
...
\WARP s and their morphisms form a category, written
$\mathbf{WARP}$.
\end{proposition}

In this paper, we write $\mathbf{WARP}$ for the category of \WARP\ graphs and \WARP\ morphisms. In later papers, where multiple categories are in play, we may refer to this category as $\mathcal{W}$ for brevity.

There is a forgetful functor
\[
  \pi : \mathbf{WARP} \to \Graph
\]
sending $(S, \alpha, \beta)$ to its skeleton $S$, sending each atom $\Atom(p)$ to the empty graph (reflecting the absence of skeletal structure at atomic leaves), and acting on morphisms by forgetting the attachment components.  This functor will later be refined to a
...
\WARP s are intended to \emph{extend} rather than replace standard graph models.
...
For the present paper, the key point is that \WARP s \emph{subsume} ordinary typed
...
formalisms such as bigraphs~\cite{Mil02} offer alternative semantics; \WARP s
are intentionally minimal while remaining adhesive-friendly.

\section{Discussion and Outlook}
\label{sec:WARP-outlook}

\WARP s provide a minimal, well-founded ``graphs all the way down'' substrate that
is compatible with adhesive graph rewriting.  This paper isolates that object
and its basic categorical properties so that later work can treat \WARP s as the
...
substrate to define deterministic multiway DPO rewriting on \WARP s, show how
...
\begin{figure}[H]
  \centering
  \DiagramNestedWARP
  \caption{Nested \WARP\ graphs: the middle vertex $w_2$ carries the inner $X_1$; all other attachments are atoms.}
