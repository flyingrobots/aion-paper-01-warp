...
\newcommand{\DiagramUnfoldingTower}{
  \begin{tikzpicture}[>=stealth, node distance=1.8cm]
    \node (x0) [warpstage] {$\mathrm{unf}_0(X)$};
    \node (x1) [warpstage, below of=x0] {$\mathrm{unf}_1(X)$};
    \node (x2) [warpstage, below of=x1] {$\mathrm{unf}_2(X)$};
    \node (xinfty) [warpstage, below of=x2] {$\mathrm{unf}_\infty(X)$};
